...
def init_database():
    ...
    # เพิ่มครูผู้สอนหลักถ้ายังไม่มี
    cursor.execute("SELECT COUNT(*) FROM teachers")
    if cursor.fetchone()[0] == 0:
        # ครูผู้สอนหลัก
        teacher_id = "T001"
        username = "admin"
        password_hash = md5("admin123")  # เปลี่ยนรหัสผ่านนี้ใน production
        fullname = "ครูผู้ดูแลระบบ"
        email = "admin@zllearning.com"
        
        cursor.execute('''
        INSERT INTO teachers (teacher_id, username, password_hash, fullname, email, role, created_at, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (teacher_id, username, password_hash, fullname, email, 'admin', 
        ...
        for student_id, fullname, email, phone in sample_students:
            registration_code = generate_registration_code()
            cursor.execute('''
            INSERT OR REPLACE INTO students 
            (student_id, fullname, email, phone, created_date, status, registration_code)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (student_id, fullname, email, phone, 
                  datetime.now().strftime("%Y-%m-%d"), 'active', registration_code))
    
    # เพิ่มคอร์สตัวอย่าง
    cursor.execute("SELECT COUNT(*) FROM courses")
    if cursor.fetchone()[0] == 0:
        sample_course = (
            "C001", "คณิตศาสตร์พื้นฐาน", "T001", "ครูผู้ดูแลระบบ",
            "คอร์สเรียนคณิตศาสตร์พื้นฐานสำหรับนักเรียนระดับมัธยม",
            "", "math-basic-room", 20, 0, "กลุ่มใหญ่", "active",
            generate_security_code(), datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        )
        
        cursor.execute('''
        INSERT INTO courses 
        (course_id, course_name, teacher_id, teacher_name, description, 
...
def check_student_id(student_id):
    ...
    try:
        ...
        cursor.execute('''
        SELECT student_id, fullname, email, registration_code 
        FROM students 
        ...
        if student:
            student_id, fullname, email, registration_code = student
            ...
            WHERE student_id = ?
            ''', (student_id,))
            attendance_count = cursor.fetchone()[0] + 1
            
            cursor.execute('''
            INSERT INTO attendance 
            (check_id, student_id, fullname, check_date, check_time, attendance_count, status)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (
                check_id, student_id, fullname,
                datetime.now().strftime("%Y-%m-%d"),
                datetime.now().strftime("%H:%M:%S"),
                attendance_count, "verified"
            ))
            
            conn.commit()
            conn.close()
            
            return True, fullname, email, registration_code
        else:
            conn.close()
...
def enroll_student_in_course(student_id, student_name, course_id, course_name):
    ...
    try:
        ...
        (enrollment_id, student_id, fullname, course_id, course_name, enrollment_date, 
         completion_status, completion_date, certificate_issued)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            enrollment_id, student_id, student_name, course_id, course_name,
...
def mark_course_completed(student_id, course_id):
    ...
    try:
        ...
        SET completion_status = 1, 
            completion_date = ?
        WHERE student_id = ? AND course_id = ?
        ''', (datetime.now().strftime("%Y-%m-%d %H:%M:%S"), student_id, course_id))
...
def teacher_login(username, password):
    ...
    try:
        ...
        cursor.execute('''
        SELECT teacher_id, fullname, password_hash 
        FROM teachers 
        ...
        if teacher:
            teacher_id, fullname, stored_hash = teacher
            password_hash = md5(password)
            
            if stored_hash == password_hash:
                conn.close()
                return True, "เข้าสู่ระบบสำเร็จ!", teacher_id, fullname
            else:
                conn.close()
...
def create_certificate(student_id, student_name, course_id, course_name, teacher_name):
    ...
    try:
        ...
        # สร้างไฟล์ใบรับรองแบบข้อความ (ในระบบจริงควรใช้ PDF)
...
def check_teacher_credentials(username, password):
    ...
    try:
        ...
        if user:
            teacher_id, fullname, stored_hash = user
            if stored_hash == md5(password):
                return True, teacher_id, fullname
...
def register_new_student(fullname, email, phone):
    ...
    try:
        ...
        student_id = f"ZLS{count + 101:03d}"
        ...
        return True, student_id, registration_code
    except Exception as e:
...
def teacher_manage_students():
    """จัดการนักเรียนโดยครูผู้สอน"""
    st.subheader("👨‍🎓 จัดการนักเรียน")
    
    tab1, tab2, tab3 = st.tabs(["📋 รายชื่อนักเรียน", "➕ เพิ่มนักเรียน", "📊 สถิติ"])
    
    with tab1:
        try:
            conn = sqlite3.connect('database.db')
            students_df = pd.read_sql_query('''
            SELECT student_id, fullname, email, phone, created_date, status 
            FROM students 
            ORDER BY created_date DESC
            ''', conn)
            conn.close()
    ...
    with tab2:
        ...
        with st.form("add_student_form", clear_on_submit=True):
            ...
            with col1:
                fullname = st.text_input("ชื่อ-สกุล *")
                email = st.text_input("อีเมล")
            with col2:
                phone = st.text_input("โทรศัพท์")
            
            submitted = st.form_submit_button("✅ เพิ่มนักเรียน", use_container_width=True)
            
            if submitted:
                if fullname:
                    success, student_id, result = register_new_student(fullname, email, phone)
                    if success:
                        st.success(f"✅ **เพิ่มนักเรียนสำเร็จ!**")
                        st.info(f"**รหัสนักเรียน:** {student_id}")
                        st.info(f"**รหัสลงทะเบียน:** {result}")
                        st.rerun()
                    else:
                        st.error(f"เกิดข้อผิดพลาด: {result}")
                else:
                    st.error("กรุณากรอกชื่อ-สกุล")
    ...
    with tab3:
        try:
            ...
            cursor.execute("SELECT COUNT(*) FROM attendance WHERE check_date = ?", (today,))
            ...
            with col1:
                st.metric("นักเรียนทั้งหมด", total_students)
            with col2:
                st.metric("นักเรียน active", active_students)
            with col3:
                st.metric("เข้าเรียนวันนี้", today_attendance)
            with col4:
                st.metric("การลงทะเบียน", total_enrollments)
...
if st.session_state.page == "student_check":
    ...
    # Tabs สำหรับนักเรียนและลงทะเบียนใหม่
    tab1, tab2 = st.tabs(["🎓 นักเรียนปัจจุบัน", "📝 ลงทะเบียนใหม่"])
            ...
            if check_btn:
                if not student_id:
                    ...
                    with st.spinner("กำลังตรวจสอบสิทธิ์..."):
                        time.sleep(1)
                        verified, student_name, student_email, reg_code = check_student_id(student_id)
    ...
    with tab2:
        ...
        with col2:
            ...
            st.write("### 📝 ลงทะเบียนนักเรียนใหม่")
            st.write("**สำหรับนักเรียนที่ยังไม่มีรหัสนักเรียน**")
            st.write("1. กรอกข้อมูลส่วนตัว")
            st.write("2. กดปุ่ม 'ลงทะเบียน'")
            st.write("3. คุณจะได้รับรหัสนักเรียนสำหรับใช้เข้าเรียน")
            st.markdown('</div>', unsafe_allow_html=True)
            ...
            with st.form("student_registration_form"):
                ...
                with col_name:
                    fullname = st.text_input("ชื่อ-สกุล *", key="reg_fullname")
                with col_contact:
                    email = st.text_input("อีเมล", key="reg_email")
                    phone = st.text_input("โทรศัพท์", key="reg_phone")
                
                terms = st.checkbox("ยอมรับเงื่อนไขและนโยบายความเป็นส่วนตัว", key="reg_terms")
                ...
                if submitted:
                    if not fullname:
                        ...
                        st.error("กรุณายอมรับเงื่อนไข")
                    else:
                        success, student_id, reg_code = register_new_student(fullname, email, phone)
                        if success:
                            st.success("✅ **ลงทะเบียนสำเร็จ!**")
                            st.info(f"**รหัสนักเรียนของคุณ:** **{student_id}**")
                            st.info(f"**รหัสลงทะเบียน:** **{reg_code}**")
                            st.warning("⚠️ **บันทึกรหัสนี้ไว้ให้ดี!** คุณจะใช้รหัสนักเรียนนี้เพื่อเข้าเรียนทุกครั้ง")
                        else:
                            st.error(f"เกิดข้อผิดพลาด: {reg_code}")
        ...
        if not enrolled_courses.empty:
            ...
            for idx, row in enrolled_courses.iterrows():
                with cols[idx % 3]:
                    ...
                    try:
                        conn = sqlite3.connect('database.db')
                        course_details = pd.read_sql_query('''
                        SELECT * FROM courses WHERE course_id = ?
                        ''', conn, params=(course_id,))
                        conn.close()
...
# -----------------------------
    ...
    with st.sidebar:
        ...
        # Teacher Menu - เพิ่มเมนูจัดการนักเรียน
        menu_options = [
            "📊 Dashboard", 
            "👨‍🎓 จัดการนักเรียน",
            "📚 จัดการคอร์ส", 
    ...
    if menu_choice == "📊 Dashboard":
        ...
        try:
            ...
            # จำนวนคอร์ส
            cursor.execute("SELECT COUNT(*) FROM courses WHERE teacher_id = ?", (st.session_state.teacher_id,))
            ...
            # จำนวนนักเรียนที่ลงทะเบียนในคอร์สของครู
            cursor.execute('''
    ...
    # ---------- MANAGE STUDENTS ----------
    elif menu_choice == "👨‍🎓 จัดการนักเรียน":
        st.title("👨‍🎓 จัดการนักเรียน")
        st.markdown("---")
            ...
            if submitted:
                if not all([course_name, jitsi_room, description]):
                    ...
                    try:
                        ...
                        cursor.execute('''
                        INSERT INTO courses 
                        (course_id, course_name, teacher_id, teacher_name, image_path, 
                         jitsi_room, description, max_students, current_students, 
                         class_type, status, security_code, created_date)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        ''', (
                            course_id, course_name, st.session_state.teacher_id, 
                            st.session_state.teacher_name, img_path, jitsi_room, 
                            description, max_students, 0, class_type, "active", 
                            security_code, datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        ))
                ...
                with col_control:
                    ...
                    with col_complete:
                        if st.button("ประกาศเรียนจบ", type="primary", key="mark_completed", use_container_width=True):
                            try:
                                student_courses_df = pd.read_excel("save_data/student_courses.xlsx")
